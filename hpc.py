...
def get_args():
    ...
    parser.add_argument(
        ...
        required=True,
        type=str
    ) ### Point to the models/PlantSegNet/ directory
...
def predict_downsampled(points, semantic_model, instance_model, method, dist=5):
    ...
    if (
        ...
        pred_semantic_label = semantic_model(torch.unsqueeze(points, 
                                                             dim=0
                                                             ).to(semantic_model.device))
    else:
        pred_semantic_label = semantic_model(torch.unsqueeze(points[:,:3], 
                                                             dim=0
                                                             ).to(semantic_model.device))
    ...
    if (
        ...
        pred_instance_features = instance_model(torch.unsqueeze(instance_points,
                                                                dim=0
                                                                ).to(instance_model.device))
    else:
        pred_instance_features = instance_model(torch.unsqueeze(instance_points[:,:3],
                                                                dim=0
                                                                ).to(instance_model.device))
...
def pred_full_size(
    ...
    semantic_full = torch.full((full_points.shape[0],),
                               -1, 
    ...
    instance_full = torch.full((focal_points.shape[0],),
                               -1, 
...
def load_model(args, model_name, version, device):
    ...
    path_all_checkpoints = os.path.join(f'{args.model}', 
                                        model_name, 
...
def worker(args, cpu_id, device, ids):
    ...
    semantic_model = load_model(args, 
                                "SorghumPartNetSemantic", 
                                args.semantic_version, device
                                ).double()
    instance_model = load_model(args, 
                                "SorghumPartNetInstance", 
    ...
    for id in tqdm(ids, file=sys.stdout):
        ...
        try:
            points_full, points, normals = load_ply_file_points(path_to_pcd, 
                                                                n_points=8000, 
                                                                full_points=8000
                                                                )
