...
def load_pcd_plyfile(path,index_to_use='leaf_index',down_sample_n=8000):
    try:
        with open(path, 'rb') as f:
            ...
            leaf_index = np.asarray(np.array(plydata.elements[2].data).tolist())[downsample_indexes].squeeze()
            leaf_part_index = np.asarray(np.array(plydata.elements[3].data).tolist())[downsample_indexes].squeeze()

            if np.min(leaf_part_index) == 1:
                leaf_part_index -=1

            is_focal_plant = np.asarray(np.array(plydata.elements[4].data).tolist())[downsample_indexes].squeeze()

            if len(plydata.elements)==6:
                plant_index = np.asarray(np.array(plydata.elements[5].data).tolist())[downsample_indexes].squeeze()
            elif len(plydata.elements)==7:
                plant_index = np.asarray(np.array(plydata.elements[5].data).tolist())[downsample_indexes].squeeze()
                ground_index = np.asarray(np.array(plydata.elements[6].data).tolist())[downsample_indexes].squeeze()
            else:
                plant_index = None
                ground_index = None
