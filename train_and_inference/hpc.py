...
def predict_downsampled(points, semantic_model, instance_model, method, dist=5):
    if (
        ...
        pred_semantic_label = semantic_model(torch.unsqueeze(points, dim=0).to(semantic_model.device))
    else:
        pred_semantic_label = semantic_model(torch.unsqueeze(points[:,:3], dim=0).to(semantic_model.device))
    ...
    if (
        ...
        pred_instance_features = instance_model(torch.unsqueeze(instance_points, dim=0).to(instance_model.device))
    else:
        pred_instance_features = instance_model(torch.unsqueeze(instance_points[:,:3], dim=0).to(instance_model.device))
